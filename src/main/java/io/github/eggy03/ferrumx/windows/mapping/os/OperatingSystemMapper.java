package io.github.eggy03.ferrumx.windows.mapping.os;

import io.github.eggy03.ferrumx.windows.entity.os.OperatingSystem;
import io.github.eggy03.ferrumx.windows.mapping.CommonMappingInterface;

/**
 * Provides a type-safe implementation of {@link CommonMappingInterface}
 * and maps JSON strings from Powershell to {@link OperatingSystem} objects or lists of {@link OperatingSystem} objects
 *
 * @since 2.3.0
 * @author Egg-03
 */
public class OperatingSystemMapper implements CommonMappingInterface<OperatingSystem> {
}
